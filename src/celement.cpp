...
void CElement::staticInit()
{
    ...
    /** 2 **/
    Kn = (sqrt(3.0) * input->Ec * thickness*1e6) / (3 * (1 - input->possion));
    Ks = Kn * (1 - 3 * input->possion);
    /** 3 **/

    /** 4 **/
    Cp = input->c*2.0*input->r*thickness;
    /** 5 **/
    Ft = input->ft * 2 *input->r * thickness*1e6;
    Fc = input->fc * 2 *input->r * thickness*1e6;
    /** 6 **/
    /** 7 **/
    m = input->density * 0.5 * PI * input->r * input->r * thickness;
    /** 8 **/
    I = 0.5 * m * input->r * input->r;
}

CElement::CElement(int number, double coordX, double coordY)
{
    //单元性质
    ...
    for (int i = 0; i < 4; i++)
    {
        vt_t2[i] = 0.0; //vt_t2和mt_t2分别表示t-△t/2时刻的速度和转动速度
        vtt2[i] = 0.0;
    }
...
void CElement::calContactForce(CElement* p2, CONTACT* cont1)//计算接触力有弹簧，p2单元作用到p1单元上的力，同时也储存到p2的接触力容器中，便于计算p2的接触力
{
   ...
   // c_y = ly + this->m_y; //接触点c的y坐标
    ...
    //cout<<"m_disn:"<<this->m_disn<<endl;
    //对应第2页第5步
    cont1->m_fn = cont1->m_fn - Kn*this->m_disn;//求法向力fn
    cont1->m_fs = cont1->m_fs + Ks*this->m_diss;//求切向力fs
    //对应第2页第6步
    cont1->m_dn=-input->beta*Kn*(this->m_disn);//求法向阻尼力Dn，beta为参数，设置为0.6
    cont1->m_ds=-input->beta*Ks*(this->m_diss);//求切向阻尼力Ds
    //对应第3页第7步
    this->tau=Cp+input->miu*(cont1->m_fn);//求极限剪力值tau，C为常数,这个tau用于后面的判断，在DiscreteElement.cpp中
    ...
    else //弹簧没有断裂,按照原来步骤计算
    {
        ...
        cont1->m_M = (cont1->m_fs + cont1->m_ds)*this->m_r;
...
void CElement::calCollisionForce(CElement* p2, CONTACT* cont1) //结算碰撞力，无弹簧 , p2 是另一个单元 cont1 是他们两个的关系 原来的p1现在是当前单元this
{
    ...
    else
    {
        ...
        delta_n=v_n*deltaTime;//相对位移增量
        delta_s=v_s*deltaTime;//相对位移增量
        ...
        cont1->m_dn=input->beta*Kn*v_n;
        cont1->m_ds=input->beta*Ks*v_s;
        ...
        {
            this->tau=(cont1->m_fn)*input->miu+Cp;
            if(abs(int(cont1->m_fs))>abs(int(this->tau)))
            {
                cont1->m_fs=abs(int(this->tau))*(cont1->m_fs/abs(int(cont1->m_fs)));//方向与原摩阻力方向一致
                cont1->m_ds=0;
            }
        }
...
void CElement::calSumF()
{
    ...
    if(this->m_number==9)
    {
        this->m_Fxsum += this->m_Feqx;
    }
    else
    {
        this->m_Fxsum=this->m_Fxsum;
    }
    this->m_Fysum=this->m_Fysum;
    this->m_Msum=this->m_Msum;
}
...
void CElement::cal_vtt2() //这个单元就是他自己，全部使用this
{
    ...
    c1 = 1 - input->alpha * deltaTime / 2;
    c2 = 1 + input->alpha * deltaTime / 2;
    if(this->m_number==0)
    {
        this->vtt2[0] = 0.0;
        this->vtt2[1] = 0.0;
